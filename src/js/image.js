...
const deleteAllImgBtn = document.querySelector(".deleteAllImgBtn");
...
screenshotBtn.addEventListener("click", async function () {
    for (let page of [...document.querySelectorAll(".pageContainer")]) {
        ...
        <a class="downloadBtn" download='handwritten.jpeg' href="${canvas.toDataURL(
            "image/jpeg",
            1
        )}" t>Download Image</a>
        `;
...
deleteAllImgBtn.addEventListener("click", function () {
    imageList.innerHTML = "";
});
