...
document.addEventListener(
    "keydown",
    (
        event //to avoid mozilla from creating divs on enter
    ) => {
        if (event.key === "Enter") {
...
function startCorrectingFrom(currPage) {
    while (currPage) {
        ...
        currPage = currPage
            ...
            .nextElementSibling?.querySelector(".page");
    }
}

correctTextBtn.addEventListener(
    "click",
    startCorrectingFrom.bind(null, document.querySelector(".page"))
);
...
function fixNoEnter() {
    //to fix the weird enter key bug in 1st page last line
...
function correctOverflow(page) {
    //corrects overflow in currentPage
    if (checkOverflow(page) && page.dataset.pageno == state.totalPages) {
        //if this page is overflowing and last page
        makeNewPage();
        state.currentPage.blur();
        // page.nextElementSibling.focus();
    }
    if (checkOverflow(page) && page.dataset.pageno != state.totalPages) {
        //if this page is overflowing and nt last page
        const nextPage = page
            .closest(".pageContainer")
            .nextElementSibling.querySelector(".page");

        while (checkOverflow(page)) {
            if (page.lastChild.nodeType != 3) {
                // its not text
                nextPage.insertAdjacentElement("afterbegin", page.lastChild); //if its br element
            } else if (page.lastChild.nodeType == 3) {
                //if its a text
                if (page.lastChild.nodeValue.length <= 1) {
                    //directly remove node
                    nextPage.insertAdjacentHTML(
                        "afterbegin",
                        page.lastChild.nodeValue
                    );
                    page.removeChild(page.lastChild);
                } //remove last character
                else {
                    const lastText = page.lastChild.nodeValue;
                    page.lastChild.nodeValue = lastText.slice(
                        0,
                        lastText.length - 1
                    );
                    nextPage.insertAdjacentHTML(
...
function makeNewPage() {
    //makes a new page and adds it to bottom
    state.increaseTotalPages();
    const newPage = `<div class="pageContainer">
    <button class="clearDrawingsBtn" type="button">
    Clear Page Drawings
</button>
        <canvas width="400" height="568" class="drawCanvas"></canvas>
...
function checkOverflow(el) {
    //checks for overflow in passed element
